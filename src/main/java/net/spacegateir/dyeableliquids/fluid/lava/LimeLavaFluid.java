...
import net.spacegateir.dyeableliquids.block.fluidblocks.ModFluidLime;
...
public abstract class LimeLavaFluid extends FlowableFluid {
    public static final float MIN_HEIGHT_TO_REPLACE = 0.44444445F;

    @Override
    public Fluid getFlowing() {
        return ModFluids.FLOWING_LIME_LAVA;
    }

    @Override
    public Fluid getStill() {
        return ModFluids.STILL_LIME_LAVA;
    }

    @Override
    public Item getBucketItem() {
        return ModItems.LIME_LAVA_BUCKET;
    }
    ...
    protected void flow(WorldAccess world, BlockPos pos, BlockState state, Direction direction, FluidState fluidState) {
        ...
        // LIME_LAVA + LIME_WATER => LIME_CONCRETE
        if (direction == Direction.DOWN) {FluidState fluidState2 = world.getFluidState(pos);
            if (this.isIn(ModTags.Fluids.LIME_LAVA_DL) && fluidState2.isIn(ModTags.Fluids.LIME_WATER_DL)) {
                if (state.getBlock() instanceof ModFluidLime) {
                    world.setBlockState(pos, Blocks.LIME_CONCRETE.getDefaultState(), Block.NOTIFY_ALL);
                }
    ...
    public BlockState toBlockState(FluidState state) {
        return ModFluids.LIME_LAVA_BLOCK.getDefaultState().with(Properties.LEVEL_15, getBlockStateLevel(state));
    }

    // Subclasses

    public static class Flowing extends LimeLavaFluid {
        @Override
    ...
    public static class Still extends LimeLavaFluid {
        @Override
        public int getLevel(FluidState state) {
            return 8;
        }
