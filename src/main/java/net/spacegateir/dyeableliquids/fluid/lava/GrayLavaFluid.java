...
import net.spacegateir.dyeableliquids.block.fluidblocks.ModFluidGray;
...
public abstract class GrayLavaFluid extends FlowableFluid {
    public static final float MIN_HEIGHT_TO_REPLACE = 0.44444445F;

    @Override
    public Fluid getFlowing() {
        return ModFluids.FLOWING_GRAY_LAVA;
    }

    @Override
    public Fluid getStill() {
        return ModFluids.STILL_GRAY_LAVA;
    }

    @Override
    public Item getBucketItem() {
        return ModItems.GRAY_LAVA_BUCKET;
    }
    ...
    protected void flow(WorldAccess world, BlockPos pos, BlockState state, Direction direction, FluidState fluidState) {
        ...
        // GRAY_LAVA + GRAY_WATER => GRAY_CONCRETE
        if (direction == Direction.DOWN) {FluidState fluidState2 = world.getFluidState(pos);
            if (this.isIn(ModTags.Fluids.GRAY_LAVA_DL) && fluidState2.isIn(ModTags.Fluids.GRAY_WATER_DL)) {
                if (state.getBlock() instanceof ModFluidGray) {
                    world.setBlockState(pos, Blocks.GRAY_CONCRETE.getDefaultState(), Block.NOTIFY_ALL);
                }
    ...
    public BlockState toBlockState(FluidState state) {
        return ModFluids.GRAY_LAVA_BLOCK.getDefaultState().with(Properties.LEVEL_15, getBlockStateLevel(state));
    }

    // Subclasses

    public static class Flowing extends GrayLavaFluid {
        @Override
    ...
    public static class Still extends GrayLavaFluid {
        @Override
        public int getLevel(FluidState state) {
            return 8;
        }
