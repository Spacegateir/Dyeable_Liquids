...
import net.spacegateir.dyeableliquids.block.fluidblocks.ModFluidPink;
...
public abstract class PinkLavaFluid extends FlowableFluid {
    ...
    public Fluid getStill() {
        return ModFluids.STILL_PINK_LAVA;
    }
    ...
    protected void flow(WorldAccess world, BlockPos pos, BlockState state, Direction direction, FluidState fluidState) {
        ...
        // PINK_LAVA + PINK_WATER => PINK_CONCRETE
        if (direction == Direction.DOWN) {FluidState fluidState2 = world.getFluidState(pos);
            if (this.isIn(ModTags.Fluids.PINK_LAVA_DL) && fluidState2.isIn(ModTags.Fluids.PINK_WATER_DL)) {
                if (state.getBlock() instanceof ModFluidPink) {
                    world.setBlockState(pos, Blocks.PINK_CONCRETE.getDefaultState(), Block.NOTIFY_ALL);
                }
    ...
    public BlockState toBlockState(FluidState state) {
        return ModFluids.PINK_LAVA_BLOCK.getDefaultState().with(Properties.LEVEL_15, getBlockStateLevel(state));
    }
    ...
    public static class Still extends PinkLavaFluid {
        @Override
        public int getLevel(FluidState state) {
            return 8;
        }
