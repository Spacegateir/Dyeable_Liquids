...
import net.spacegateir.dyeableliquids.block.fluidblocks.ModFluidRed;
...
public abstract class RedLavaFluid extends FlowableFluid {
    public static final float MIN_HEIGHT_TO_REPLACE = 0.44444445F;

    @Override
    public Fluid getFlowing() {
        return ModFluids.FLOWING_RED_LAVA;
    }

    @Override
    public Fluid getStill() {
        return ModFluids.STILL_RED_LAVA;
    }

    @Override
    public Item getBucketItem() {
        return ModItems.RED_LAVA_BUCKET;
    }
    ...
    protected void flow(WorldAccess world, BlockPos pos, BlockState state, Direction direction, FluidState fluidState) {
        ...
        // RED_LAVA + RED_WATER => RED_CONCRETE
        if (direction == Direction.DOWN) {FluidState fluidState2 = world.getFluidState(pos);
            if (this.isIn(ModTags.Fluids.RED_LAVA_DL) && fluidState2.isIn(ModTags.Fluids.RED_WATER_DL)) {
                if (state.getBlock() instanceof ModFluidRed) {
                    world.setBlockState(pos, Blocks.RED_CONCRETE.getDefaultState(), Block.NOTIFY_ALL);
                }
    ...
    public BlockState toBlockState(FluidState state) {
        return ModFluids.RED_LAVA_BLOCK.getDefaultState().with(Properties.LEVEL_15, getBlockStateLevel(state));
    }
    ...
    public static class Still extends RedLavaFluid {
        @Override
        public int getLevel(FluidState state) {
            return 8;
        }
