...
import net.spacegateir.dyeableliquids.block.fluidblocks.ModFluidCyan;
...
public abstract class CyanLavaFluid extends FlowableFluid {
    ...
    public Fluid getFlowing() {
        return ModFluids.FLOWING_CYAN_LAVA;
    }

    @Override
    public Fluid getStill() {
        return ModFluids.STILL_CYAN_LAVA;
    }

    @Override
    public Item getBucketItem() {
        return ModItems.CYAN_LAVA_BUCKET;
    }
        ...
        if (direction == Direction.DOWN) {FluidState fluidState2 = world.getFluidState(pos);
            if (this.isIn(ModTags.Fluids.CYAN_LAVA_DL) && fluidState2.isIn(ModTags.Fluids.CYAN_WATER_DL)) {
                if (state.getBlock() instanceof ModFluidCyan) {
                    world.setBlockState(pos, Blocks.CYAN_CONCRETE.getDefaultState(), Block.NOTIFY_ALL);
                }
    ...
    public BlockState toBlockState(FluidState state) {
        return ModFluids.CYAN_LAVA_BLOCK.getDefaultState().with(Properties.LEVEL_15, getBlockStateLevel(state));
    }

    // Subclasses

    public static class Flowing extends CyanLavaFluid {
        @Override
    ...
    public static class Still extends CyanLavaFluid {
        @Override
        public int getLevel(FluidState state) {
            return 8;
        }
