...
import net.spacegateir.dyeableliquids.block.fluidblocks.ModFluidLightBlue;
...
public abstract class LightBlueLavaFluid extends FlowableFluid {
    public static final float MIN_HEIGHT_TO_REPLACE = 0.44444445F;

    @Override
    public Fluid getFlowing() {
        return ModFluids.FLOWING_LIGHT_BLUE_LAVA;
    }

    @Override
    public Fluid getStill() {
        return ModFluids.STILL_LIGHT_BLUE_LAVA;
    }

    @Override
    public Item getBucketItem() {
        return ModItems.LIGHT_BLUE_LAVA_BUCKET;
    }
    ...
    protected void flow(WorldAccess world, BlockPos pos, BlockState state, Direction direction, FluidState fluidState) {
        ...
        // LIGHT_BLUE_LAVA + LIGHT_BLUE_WATER => LIGHT_BLUE_CONCRETE
        if (direction == Direction.DOWN) {FluidState fluidState2 = world.getFluidState(pos);
            if (this.isIn(ModTags.Fluids.LIGHT_BLUE_LAVA_DL) && fluidState2.isIn(ModTags.Fluids.LIGHT_BLUE_WATER_DL)) {
                if (state.getBlock() instanceof ModFluidLightBlue) {
                    world.setBlockState(pos, Blocks.LIGHT_BLUE_CONCRETE.getDefaultState(), Block.NOTIFY_ALL);
                }
    ...
    public BlockState toBlockState(FluidState state) {
        return ModFluids.LIGHT_BLUE_LAVA_BLOCK.getDefaultState().with(Properties.LEVEL_15, getBlockStateLevel(state));
    }
    ...
    public static class Still extends LightBlueLavaFluid {
        @Override
        public int getLevel(FluidState state) {
            return 8;
        }
