...
import net.spacegateir.dyeableliquids.block.fluidblocks.ModFluidBrown;
...
public abstract class BrownLavaFluid extends FlowableFluid {
    public static final float MIN_HEIGHT_TO_REPLACE = 0.44444445F;

    @Override
    public Fluid getFlowing() {
        return ModFluids.FLOWING_BROWN_LAVA;
    }

    @Override
    public Fluid getStill() {
        return ModFluids.STILL_BROWN_LAVA;
    }

    @Override
    public Item getBucketItem() {
        return ModItems.BROWN_LAVA_BUCKET;
    }
    ...
    protected void flow(WorldAccess world, BlockPos pos, BlockState state, Direction direction, FluidState fluidState) {
        ...
        // BROWN_LAVA + BROWN_WATER => BROWN_CONCRETE
        if (direction == Direction.DOWN) {FluidState fluidState2 = world.getFluidState(pos);
            if (this.isIn(ModTags.Fluids.BROWN_LAVA_DL) && fluidState2.isIn(ModTags.Fluids.BROWN_WATER_DL)) {
                if (state.getBlock() instanceof ModFluidBrown) {
                    world.setBlockState(pos, Blocks.BROWN_CONCRETE.getDefaultState(), Block.NOTIFY_ALL);
                }
    ...
    public BlockState toBlockState(FluidState state) {
        return ModFluids.BROWN_LAVA_BLOCK.getDefaultState().with(Properties.LEVEL_15, getBlockStateLevel(state));
    }
    ...
    public static class Still extends BrownLavaFluid {
        @Override
        public int getLevel(FluidState state) {
            return 8;
        }
