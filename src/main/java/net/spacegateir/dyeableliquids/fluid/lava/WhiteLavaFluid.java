...
import net.spacegateir.dyeableliquids.block.fluidblocks.ModFluidWhite;
...
public abstract class WhiteLavaFluid extends FlowableFluid {
    public static final float MIN_HEIGHT_TO_REPLACE = 0.44444445F;

    @Override
    public Fluid getFlowing() {
        return ModFluids.FLOWING_WHITE_LAVA;
    }

    @Override
    public Fluid getStill() {
        return ModFluids.STILL_WHITE_LAVA;
    }

    @Override
    public Item getBucketItem() {
        return ModItems.WHITE_LAVA_BUCKET;
    }
    ...
    protected void flow(WorldAccess world, BlockPos pos, BlockState state, Direction direction, FluidState fluidState) {
        ...
        // WHITE_LAVA + WHITE_WATER => WHITE_CONCRETE
        if (direction == Direction.DOWN) {FluidState fluidState2 = world.getFluidState(pos);
            if (this.isIn(ModTags.Fluids.WHITE_LAVA_DL) && fluidState2.isIn(ModTags.Fluids.WHITE_WATER_DL)) {
                if (state.getBlock() instanceof ModFluidWhite) {
                    world.setBlockState(pos, Blocks.WHITE_CONCRETE.getDefaultState(), Block.NOTIFY_ALL);
                }
    ...
    public BlockState toBlockState(FluidState state) {
        return ModFluids.WHITE_LAVA_BLOCK.getDefaultState().with(Properties.LEVEL_15, getBlockStateLevel(state));
    }
    ...
    public static class Still extends WhiteLavaFluid {
        @Override
        public int getLevel(FluidState state) {
            return 8;
        }
