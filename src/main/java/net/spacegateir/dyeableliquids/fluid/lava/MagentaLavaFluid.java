package net.spacegateir.dyeableliquids.fluid.lava;
...
import net.spacegateir.dyeableliquids.block.fluidblocks.ModFluidMagenta;
...
public abstract class MagentaLavaFluid extends FlowableFluid {
    public static final float MIN_HEIGHT_TO_REPLACE = 0.44444445F;

    @Override
    public Fluid getFlowing() {
        return ModFluids.FLOWING_MAGENTA_LAVA;
    }

    @Override
    public Fluid getStill() {
        return ModFluids.STILL_MAGENTA_LAVA;
    }

    @Override
    public Item getBucketItem() {
        return ModItems.MAGENTA_LAVA_BUCKET;
    }
    ...
    protected void flow(WorldAccess world, BlockPos pos, BlockState state, Direction direction, FluidState fluidState) {
        ...
        // MAGENTA_LAVA + MAGENTA_WATER => MAGENTA_CONCRETE
        if (direction == Direction.DOWN) {FluidState fluidState2 = world.getFluidState(pos);
            if (this.isIn(ModTags.Fluids.MAGENTA_LAVA_DL) && fluidState2.isIn(ModTags.Fluids.MAGENTA_WATER_DL)) {
                if (state.getBlock() instanceof ModFluidMagenta) {
                    world.setBlockState(pos, Blocks.MAGENTA_CONCRETE.getDefaultState(), Block.NOTIFY_ALL);
                }
    ...
    public BlockState toBlockState(FluidState state) {
        return ModFluids.MAGENTA_LAVA_BLOCK.getDefaultState().with(Properties.LEVEL_15, getBlockStateLevel(state));
    }

    // Subclasses

    public static class Flowing extends MagentaLavaFluid {
        @Override
    ...
    public static class Still extends MagentaLavaFluid {
        @Override
        public int getLevel(FluidState state) {
            return 8;
        }
