...
import net.spacegateir.dyeableliquids.block.fluidblocks.ModFluidGreen;
...
public abstract class GreenLavaFluid extends FlowableFluid {
    ...
    public Item getBucketItem() {
        return ModItems.GREEN_LAVA_BUCKET;
    }
    ...
    protected void flow(WorldAccess world, BlockPos pos, BlockState state, Direction direction, FluidState fluidState) {
        ...
        // GREEN_LAVA + GREEN_WATER => GREEN_CONCRETE
        if (direction == Direction.DOWN) {FluidState fluidState2 = world.getFluidState(pos);
            if (this.isIn(ModTags.Fluids.GREEN_LAVA_DL) && fluidState2.isIn(ModTags.Fluids.GREEN_WATER_DL)) {
                if (state.getBlock() instanceof ModFluidGreen) {
                    world.setBlockState(pos, Blocks.GREEN_CONCRETE.getDefaultState(), Block.NOTIFY_ALL);
                }
    ...
    public BlockState toBlockState(FluidState state) {
        return ModFluids.GREEN_LAVA_BLOCK.getDefaultState().with(Properties.LEVEL_15, getBlockStateLevel(state));
    }
    ...
    public static class Still extends GreenLavaFluid {
        @Override
        public int getLevel(FluidState state) {
            return 8;
        }
