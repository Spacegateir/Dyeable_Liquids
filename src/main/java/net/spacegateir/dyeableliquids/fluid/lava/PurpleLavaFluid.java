...
import net.spacegateir.dyeableliquids.block.fluidblocks.ModFluidPurple;
...
public abstract class PurpleLavaFluid extends FlowableFluid {
    public static final float MIN_HEIGHT_TO_REPLACE = 0.44444445F;

    @Override
    public Fluid getFlowing() {
        return ModFluids.FLOWING_PURPLE_LAVA;
    }

    @Override
    public Fluid getStill() {
        return ModFluids.STILL_PURPLE_LAVA;
    }

    @Override
    public Item getBucketItem() {
        return ModItems.PURPLE_LAVA_BUCKET;
    }
    ...
    protected void flow(WorldAccess world, BlockPos pos, BlockState state, Direction direction, FluidState fluidState) {
        ...
        // PURPLE_LAVA + PURPLE_WATER => PURPLE_CONCRETE
        if (direction == Direction.DOWN) {FluidState fluidState2 = world.getFluidState(pos);
            if (this.isIn(ModTags.Fluids.PURPLE_LAVA_DL) && fluidState2.isIn(ModTags.Fluids.PURPLE_WATER_DL)) {
                if (state.getBlock() instanceof ModFluidPurple) {
                    world.setBlockState(pos, Blocks.PURPLE_CONCRETE.getDefaultState(), Block.NOTIFY_ALL);
                }
    ...
    public BlockState toBlockState(FluidState state) {
        return ModFluids.PURPLE_LAVA_BLOCK.getDefaultState().with(Properties.LEVEL_15, getBlockStateLevel(state));
    }
    ...
    public static class Still extends PurpleLavaFluid {
        @Override
        public int getLevel(FluidState state) {
            return 8;
        }
