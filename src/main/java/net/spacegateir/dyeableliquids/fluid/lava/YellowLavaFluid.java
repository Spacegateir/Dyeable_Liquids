...
import net.spacegateir.dyeableliquids.block.fluidblocks.ModFluidYellow;
...
public abstract class YellowLavaFluid extends FlowableFluid {
    public static final float MIN_HEIGHT_TO_REPLACE = 0.44444445F;

    @Override
    public Fluid getFlowing() {
        return ModFluids.FLOWING_YELLOW_LAVA;
    }

    @Override
    public Fluid getStill() {
        return ModFluids.STILL_YELLOW_LAVA;
    }

    @Override
    public Item getBucketItem() {
        return ModItems.YELLOW_LAVA_BUCKET;
    }
    ...
    protected void flow(WorldAccess world, BlockPos pos, BlockState state, Direction direction, FluidState fluidState) {
        ...
        // YELLOW_LAVA + YELLOW_WATER => YELLOW_CONCRETE
        if (direction == Direction.DOWN) {FluidState fluidState2 = world.getFluidState(pos);
            if (this.isIn(ModTags.Fluids.YELLOW_LAVA_DL) && fluidState2.isIn(ModTags.Fluids.YELLOW_WATER_DL)) {
                if (state.getBlock() instanceof ModFluidYellow) {
                    world.setBlockState(pos, Blocks.YELLOW_CONCRETE.getDefaultState(), Block.NOTIFY_ALL);
                }
    ...
    public BlockState toBlockState(FluidState state) {
        return ModFluids.YELLOW_LAVA_BLOCK.getDefaultState().with(Properties.LEVEL_15, getBlockStateLevel(state));
    }
    ...
    public static class Still extends YellowLavaFluid {
        @Override
        public int getLevel(FluidState state) {
            return 8;
        }
